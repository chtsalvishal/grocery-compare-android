...
class HomeViewModel(
    ...
    val uiState: StateFlow<HomeUiState> = combine(
        ...
        val warmingUp   = args[2] as Boolean
        val progress    = args[3] as? ScrapeProgress
        val selectedCat = args[4] as String
        val query       = args[5] as String
        val user        = args[6] as UserEntity
        @Suppress("UNCHECKED_CAST")
        val stores      = args[7] as Set<String>
        val sort        = args[8] as String
    ...
    fun triggerScrape() {
        ...
        workManager.enqueueUniqueWork("grocery_sync", ExistingWorkPolicy.REPLACE, syncRequest)

        // Show "warming up" banner if server hasn't responded meaningfully after 8 s
        viewModelScope.launch {
            delay(8_000L)
            if (_isRefreshing.value && (_scrapeProgress.value?.current ?: 0) < 10) {
            ...
            withTimeoutOrNull(10 * 60 * 1000L) {
                workManager.getWorkInfosByTagLiveData("grocery_sync").asFlow()
                    .collect { workInfos ->
                        ...
                        // Dismiss warm-up banner once real progress arrives
                        if (progressPct >= 10) _isWarmingUp.value = false
